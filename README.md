...
| **Integritäts-Lock (PR-Gate + Engine)** | Siegel über den Kern (42 Sperrdateien, Klassen KRITISCH/FEST): `.github/workflows/integrity-lock.yml` prüft bei jedem PR auf `main` mit `integrity_guard.py --gate` (fail-closed, Herkunft + Reparaturzeile im Log). Die Engine signiert als ersten Schritt belegten, committeten **FEST**-Drift selbst (`--heal`, mit Herkunfts-Akte im Lock); KRITISCH und Laufzeit-Mutationen bleiben HARD STOP. Anlass: Issue #316 — ein vergessener Lock kostete zwei Produktions-Slots (Details: `docs/INCIDENT-2026-09-19-integritaets-lock.md`) |

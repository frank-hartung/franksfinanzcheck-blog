...
| **Integritäts-Lock (PR-Gate + Engine)** | Siegel über den Kern (42 Sperrdateien, Klassen KRITISCH/FEST): `.github/workflows/integrity-lock.yml` prüft bei jedem PR auf `main` mit `integrity_guard.py --gate` (fail-closed, Herkunft + Reparaturzeile im Log). Der Job meldet sich als Pflicht-Check **`Integritäts-Siegel`** — der Name ist ein Vertrag mit dem Ruleset (Governance-Regel **C18**: Konstante = Workflow = Ruleset, kein Pfadfilter, kein `if:`, kein `continue-on-error`), und die Live-Wache `pflichtcheck_guard.py` prüft im Gate selbst, ob `main` den Check wirklich verlangt (Runbook: `docs/PFLICHT-CHECK-RUNBOOK.md`). Die Engine signiert als ersten Schritt belegten, committeten **FEST**-Drift selbst (`--heal`, mit Herkunfts-Akte im Lock); KRITISCH und Laufzeit-Mutationen bleiben HARD STOP. Seit 20.09. ist der Status des Pflicht-Checks selbst als **Dauerzustand dokumentiert**: `main` verlangt den Check nicht, die Ruleset-Änderung bleibt Admin-Aufgabe (C15), und das rote Kreuz hielt nachweislich keinen Merge auf (PR #327 wurde bei `FAILURE` gemergt) – `pflichtcheck_guard.py` meldet genau diesen Befund daher als BEKANNT (Exit 0, `::warning::`, 🛑 bleibt im Log) statt als Vorfall, alles andere bleibt rot, `--strict` und die Prüffrist 31.12.2026 begrenzen die Freigabe (Runbook-Abschnitt „Dauerzustand“). Anlass: Issue #316 — ein vergessener Lock kostete zwei Produktions-Slots (Details: `docs/INCIDENT-2026-09-19-integritaets-lock.md`) |
...
python3 scripts/ff_voice_backends.py --selftest  # 106 Gates: Aussprache, Prosodie, Audio, Profile, Edge-Timeout + Wortuhr-Schalter (10.09.), Aufruf-/Segment-Budget (11.09.)
python3 scripts/ff_voice_audio.py --selftest     # 119 Gates: Block-Parität, Tabellen, Injektion, Wortuhr, Segment-Budget + Deadline-Durchreichung (11.09.)

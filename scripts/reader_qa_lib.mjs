...
export function buildPage({ title, description, kurzantwort, readingTime, wordCount, lang = 'de', author = 'Frank Hartung', date = '03.09.2026', updated = '', category = 'Ratgeber', siteName = 'FranksFinanzcheck', bodyHtml, showKurzantwortBox = true, audioCfg = null }) {
  ...
  const audioConfig = audioCfg
    ? `\n<script type="application/json" id="ff-reader-audio-config">${JSON.stringify({ audio: audioCfg })}</script>`
    : '';
...
<script type="application/json" id="ff-reader-config">${JSON.stringify(cfg)}</script>${audioConfig}
  ...
  doc.createElement = function (tag, opts) {
    ...
    if (lower === 'audio') {
      ...
      Object.defineProperty(el, 'src', {
        ...
        set: (v) => { if (v) el.setAttribute('src', String(v)); },
        configurable: true
      });

...
#  VOLLSTÄNDIGE deutsche Groß-/Kleinschreibung – 17 Regeln, deterministisch
...
MASK_PATTERNS = [
    # Linkziel und Bildpfad sind URL-Raum und damit unantastbar; der sichtbare
    # Markdown-Linktext bleibt dagegen Prosa. So kann C17
    # `[Frugalismus Tipps](../slug/)` sauber beschriften, ohne je den Slug zu
    # verändern. Die frühere Reihenfolge maskierte bei HTTPS die schließende
    # Klammer mit und machte dieses Verhalten vom Linktyp abhängig.
    re.compile(r"(?<=\]\()[^)\n]+(?=\))"),
...
ABBR_STEMS = ("vs", "bzw", "etc", "usw", "ca", "circa", "Nr", "Abs", "Art", "Mio",
              ...
              "mW", "aAO", "sO", "uU", "bzgl", "Dr", "Prof", "Bd", "Bsp", "Tel",
              "Jan", "Feb", "Mär", "Apr", "Jun", "Jul", "Aug", "Sep", "Okt", "Nov", "Dez",
              # Kurzformen aus Rechtsgrundlagen und Nutzwert-Texten. Ohne diese
              # Entscheidung würde C7 in „lit. f DSGVO“, „max. ein Tag“ und
              # „Tagesgeld & Co. clever nutzen“ grammatisch korrekte Kleinschreibung
              # in Satzanfänge verwandeln.
              "lit", "max", "min", "Co")
# Ein einzelner Buchstabe plus Punkt ("z.", "B.") oder EIN EXAKTER
# Abkürzungsstamm beendet keinen Satz. Die Anker sind entscheidend: das
# frühere `search()` fand das abschließende "t." in *jedem* Wort wie
# "passt." und deaktivierte C7 im gesamten Fließtext.
ABBR_END = re.compile(r"^(?:[A-Za-zÀ-ÿ]|" + "|".join(map(re.escape, ABBR_STEMS))
                      + r")\.$", re.I)


def _is_abbreviation_end(last: str) -> bool:
    """Ist das letzte Token eine geschützte Abkürzung?

    Markdown- und HTML-Quelltext kann den Token umschließen: `(inkl.` oder
    `z.&nbsp;B.`. Erst nach dem Entschärfen dieser reinen Darstellungssyntax
    darf die Satzanfangsregel urteilen; andernfalls würde sie Rechtszitate,
    Maße und die übliche „& Co.“-Form beschädigen.
    """
    plain = unescape(last).strip(" \t\r\n([{\"'„“”«»*_`")
    tail = plain.split()[-1] if plain.split() else ""
    return bool(ABBR_END.fullmatch(tail))
...
def r_c7(text: str, hits: Sink, line: int) -> str:
    ...
    for m in C7_RE.finditer(text):
        # Das Zielwort muss vor jeder Schutzentscheidung vorliegen. Die
        # Reihenfolge ist wichtig: Eine frühere Fassung las `word` erst nach
        # der TLD-Bremse und brach damit beim ersten echten Satzanfang mit
        # `UnboundLocalError` ab – ausgerechnet im Bestandslauf.
        word = m.group(3)
        before = text[:m.end(1)]                 # Inkl. Satzzeichen – sonst
        words = before.split()                    # sieht „Z. B.“ aus wie „… Z.“
        last = words[-1] if words else ""
        if last and (_is_abbreviation_end(last) or re.search(r"[\d]\.$", last)):
            continue
...
def r_c17(text: str, hits: Sink, line: int) -> str:
    ...
    matches = list(tc.COMPOUND_RE.finditer(text))
    if not matches:
        return text
    out, geaendert = tc.join_compounds(text)
    if not geaendert:  # Defensiv: Kanon und Ersetzung dürfen nie auseinanderlaufen.
        return text
    for match in matches:
        alt = match.group(0)
        hits.add("C17", alt, tc.COMPOUND_JOIN[alt.lower()], line)
    return out
...
def apply_rules(text: str, allowed, hits: Sink, line: int = 0,
    ...
    # Erst die unveränderlichen Register maskieren, DANN die Sonderformen der
    # Marke FRITZ! bearbeiten. `r_specials` muss zwar vor dem regulären C9-Scan
    # laufen, darf aber niemals aus einem Bildpfad oder einer Link-URL
    # `fritzbox.jpg` → `FRITZ!Box.jpg` machen.
    masked, store = mask(text)
    if "C9" in allowed:
        masked = r_specials(masked, hits, line)
...
# ---------------------------------------------------------------------------
def write_report(rows, mode, scanned, scanned_words):
    total = sum(len(r["funde"]) for r in rows)
    erkannt = sum(r.get("erkannte_befunde", len(r["funde"])) for r in rows)
    hart = sum(1 for r in rows for f in r["funde"] if f["regel"] in HARD_RULES)
    gefixt = sum(r.get("gefixt", 0) for r in rows)
    # Die Quote bezieht sich auf den gesamten geprüften Content-Bestand, nicht
    # nur auf Dateien, die einmal einen Fund hatten. Sonst würde ein einzelner
    # Restfehler künstlich großgerechnet – und ein vollständig geheilter Lauf
    # hätte gar keinen belastbaren Nenner mehr.
    worte = max(scanned_words, 1)
    wortzahl = f"{scanned_words:,}".replace(",", ".")
    L = ["# 🔠 CASING-REPORT (Groß-/Kleinschreibung)", "",
         f"**Stand:** {datetime.now(timezone.utc):%Y-%m-%d %H:%M} UTC · Modus: {mode}", "",
         f"Geprüfte Dateien: {scanned} · geprüfte Content-Wörter: {wortzahl} · "
         f"Dateien mit Fund im Lauf: {len(rows)} · erkannte Funde: {erkannt} · "
         f"offene Befunde: {total} · hart (gate-würdig): {hart} · "
         f"automatisch geheilt: {gefixt}",
         "",
         f"**Casing-Deliktquote:** {1000 * hart / worte:.2f} harte Befunde je 1.000 Wörter "
         "– Zielwert 0,00 (Redaktions-Standard Capital/WiWo/ZEIT).", ""]
    open_rows = [r for r in rows if r["funde"]]
    healed_rows = [r for r in rows if r.get("gefixt")]
    pro = {}
    for r in open_rows:
        for f in r["funde"]:
            pro[f["regel"]] = pro.get(f["regel"], 0) + 1
    if pro:
        L += ["| Regel | Bedeutung | offene Befunde |", "|---|---|---|"]
        for k in sorted(pro):
    ...
    if healed_rows:
        L += ["## ✅ In diesem Lauf geheilt", "", "| Datei | erkannte Funde | Korrekturen |",
              "|---|---:|---:|"]
        for r in healed_rows[:45]:
            L.append(f"| `{r['pfad']}` | {r.get('erkannte_befunde', 0)} | {r['gefixt']} |")
        if len(healed_rows) > 45:
            L.append(f"| … | {len(healed_rows) - 45} weitere Dateien | |")
        L.append("")
    if open_rows:
        L += ["## Offene Befunde je Datei", ""]
        for r in open_rows[:45]:
        ...
        L.append("🎉 Groß-/Kleinschreibung sauber (C1–C17 + T1: Duden, Marken- und "
...
def log_history(rows, geparkt, scanned, scanned_words):
    """Schreibt einen belastbaren Lauf-Snapshot für Trend und Scorecard.

    `dateien` bleibt für ältere Auswertungen die Anzahl der Dateien mit Fund;
    die expliziten `gepruefte_*`-Felder liefern künftig die echte Grundgesamtheit
    für die Deliktquote.
    """
    ...
    with HISTORY.open("a", encoding="utf-8") as fh:
        fh.write(json.dumps({
            ...
            "dateien": len(rows),
            "gepruefte_dateien": scanned,
            "gepruefte_woerter": scanned_words,
            "erkannte_befunde": sum(r.get("erkannte_befunde", len(r["funde"])) for r in rows),
...
FIX_CASES = [
    ...
    # Regression: C7 muss einen echten Satzanfang im laufenden Absatz heilen.
    # Der Fall schützt zugleich die Reihenfolge der TLD-/Abkürzungsbremsen in
    # r_c7 – vor dem Fix wurde `word` dort vor seiner Zuweisung gelesen.
    ("Der Tarif passt. du sparst weiter.", "Der Tarif passt. Du sparst weiter.",
     BODY_RULES, True, False),
...
KEEP_CASES = [
    ...
    # URL-/Dateinamen sind selbst dann tabu, wenn ihr Text zufällig eine
    # Markenvariante enthält; sichtbar ist ausschließlich der Linktext.
    ("![Router](images/fritzbox.jpg)", BODY_RULES),
    ("https://example.invalid/fritzbox/", BODY_RULES),
    ...
    # C7-Schutz: Abkürzungen bleiben innerhalb des Satzes klein – auch wenn
    # sie durch Klammern oder HTML-NBSP formatiert sind.
    ("Art. 6 Abs. 1 lit. f DSGVO", BODY_RULES),
    ("Tagesgeld & Co. clever nutzen", BODY_RULES),
    ("Umschaltung max. ein Kalendertag", BODY_RULES),
    ("(inkl. deliktsunfähigen Kindern)", BODY_RULES),
    ("z.&nbsp;B. für ein Auto", BODY_RULES),
...
FIX_HEAD_CASES = [
    ...
    # Sichtbarer Linktext darf C17 folgen; das Linkziel bleibt bytegleich.
    ("## [Frugalismus Tipps](../../posts/frugalismus-tipps/)",
     "## [Frugalismus-Tipps](../../posts/frugalismus-tipps/)", BODY_RULES),
]
...
def main():
    ...
    rows = []
    files = target_files()
    # Einmal am unveränderten Laufbestand messen. Der Wert bleibt auch dann
    # stabil, wenn `--fix` eine Datei heilt, und ist die SSOT für Report,
    # Historie, JSON und die Chefredakteur-Scorecard.
    scanned_words = sum(words_of(p) for p in files)
    for p in files:
        found, new_text = process_file(p)
        if not found:
            continue
        row = {"pfad": str(p.relative_to(ROOT)), "funde": found,
               "erkannte_befunde": len(found), "woerter": words_of(p)}
    ...
    if DO_PLAN and PLAN_FILE.exists():
        ...
        if found:
            row = {"pfad": str(PLAN_FILE.relative_to(ROOT)), "funde": found,
                   "erkannte_befunde": len(found), "woerter": words_of(PLAN_FILE)}
            heilbar = [h for h in found if not h["regel"].endswith("-report")]
    ...
    if not DRY_RUN:
        # Erst den Snapshot anhängen, dann den Report rendern: Nur so zeigt
        # dessen Trend wirklich den gerade abgeschlossenen Lauf statt zwei
        # historische Vorgänger.
        log_history(rows, geparkt, len(files), scanned_words)
        write_report(rows, mode, len(files), scanned_words)
    if DO_JSON:
        JSON_OUT.write_text(json.dumps({
            "stand": f"{datetime.now(timezone.utc):%Y-%m-%dT%H:%M:%SZ}",
            "modus": mode,
            # `dateien` ist aus Kompatibilitätsgründen die Fundmenge; die
            # expliziten Felder darunter sind die Grundgesamtheit des Laufs.
            "dateien": len(rows),
            "gepruefte_dateien": len(files),
            "woerter": scanned_words,
            "erkannte_befunde": sum(r.get("erkannte_befunde", len(r["funde"])) for r in rows),
            "befunde": total,
            "hart": sum(1 for r in rows for f in r["funde"] if f["regel"] in HARD_RULES),
            "gefixt": sum(r.get("gefixt", 0) for r in rows), "geparkt": geparkt,
            "dateien_liste": [{"pfad": r["pfad"],
                               "erkannte_befunde": r.get("erkannte_befunde", len(r["funde"])),
                               "befunde": len(r["funde"]), "woerter": r["woerter"]}
                              for r in rows],

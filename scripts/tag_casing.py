...
#      · scripts/casing_guard.py     – damals nur C1–C3, Front-Matter weitgehend tabu
...
# Nur vollständige Wortteile koppeln. Ohne diese Grenzen würde etwa
# „Frugalismus Tippst du …“ als Präfix von „Frugalismus Tipps“ getroffen und
# zu einem erfundenen Kompositum umgebaut. C17 entscheidet bewusst nur über
# den endlichen, nachgewiesenen Kanon – nie über Wortfragmente.
COMPOUND_RE = re.compile(
    r"(?<![A-Za-zÀ-ÿ0-9])(?:"
    + "|".join(re.escape(k) for k in sorted(COMPOUND_JOIN, key=len, reverse=True))
    + r")(?![A-Za-zÀ-ÿ0-9])",
    re.I,
)
...
def join_compounds(text: str):
    ...
    if not text:
        return text, False
    out = COMPOUND_RE.sub(lambda m: COMPOUND_JOIN[m.group(0).lower()], text)
    return out, out != text
...
JOIN_CASES = [
    ...
    ("Welche weiteren Strom sparen Tipps helfen sofort?",
     "Welche weiteren Strom sparen Tipps helfen sofort?"),
    # Wortgrenze: "Tippst" enthält zwar den String "Tipps", ist aber kein
    # Kompositum aus dem C17-Kanon und darf daher niemals geklebt werden.
    ("Frugalismus Tippst du weiter?", "Frugalismus Tippst du weiter?"),
]

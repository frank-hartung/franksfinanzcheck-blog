...
t.group('12) CSS-Lesbarkeit (Kontrast, Zeilenlänge, Dunkelmodus)');
{
  ...
  t.ok(/\[data-theme="dark"\]\s*\.ff-kurzantwort/.test(css), 'Kurzantwort-Box hat eine Dunkelmodus-Variante (data-theme=dark)');
  t.ok(/#10211B/.test(css) && /#E8F1EC/.test(css), 'Dunkle Kurzantwort-Palette gesetzt (#10211B / #E8F1EC)');
  ...
  // Kernkontraste der Kurzfassung (Farben der gemergten v5-Fassung + Dunkelmodus)
  const checks = [
    ['Kasten-Label hell (#0E5A43 auf #F5FAF7)', '#0E5A43', '#F5FAF7', 7],
    ['Kasten-Text hell (#2E2E33 auf #F5FAF7)', '#2E2E33', '#F5FAF7', 7],
  ...
  t.ok(/max-(?:inline-)?size:\s*68ch/.test(css) || /max-width:\s*68ch/.test(css),
    'Kurzantwort-Text begrenzt die Zeilenlänge auf 68ch');
  t.ok(/line-height:\s*1\.68/.test(css) && /text-wrap:\s*pretty/.test(css),
    'Kurzantwort-Text: 1,68er Zeilenabstand und text-wrap:pretty (Verlagshaus-Typografie)');

  // Inline-Styling darf nicht zurückkehren; Barrierefreiheit (role=note + aria-labelledby)
  const kaTemplates = ['single.html', path.join('_default', 'single.html')].map((rel) =>
    fs.readFileSync(path.join(ROOT, 'layouts', rel), 'utf8').match(/<div class="ff-kurzantwort"[\s\S]*?\n  <\/div>/));
  t.eq(kaTemplates.map((b) => !!b && !/style="/.test(b[0]) && /role="note"/.test(b[0])
        && /aria-labelledby="ff-kurzantwort-label-/.test(b[0])),
    [true, true],
    'Kurzantwort-Box (single.html & _default/single.html): ohne Inline-Styling, mit role=note + aria-labelledby');
}

/* ================================================================== */
/* 13) ZEIT-Standard-Audiofassung (v7: #ff-reader-audio-config)      */
/* ================================================================== */
t.group('13) ZEIT-Standard-Audiofassung (HTML5-Tonspur hat Vorrang)');
{
  /* v7-Vertrag: Der Deploy-Generator (scripts/generate_reader_audio.py)
     injiziert nach dem Hugo-Build
       <script id="ff-reader-audio-config">{ "audio": { src, chunks } }</script>.
     chunks: [{ b: Block-Index in Lesereihenfolge (0 = Anmoderation),
                t0, t1 (ms), lang }]. Die Tonspur läuft im nativen
     HTML5-<audio>-Element; Web Speech wird dabei NICHT bemüht. Fehlt die
     Tonspur, bleibt die Web-Speech-Stufe automatisch aktiv. */
  const mkAudioPage = async (withAudio = true) => createPage({
    html: buildPage({
      title: art.title, description: art.description, kurzantwort: art.kurzantwort,
      readingTime: art.readingTime, wordCount: art.wordCount, author: art.author,
      bodyHtml: art.bodyHtml,
      audioCfg: withAudio ? {
        src: '/audio/articles/test-artikel.mp3',
        chunks: [
          { b: 0, t0: 0, t1: 6000, lang: 'de' },
          { b: 1, t0: 6000, t1: 12000, lang: 'de' },
          { b: 2, t0: 12000, t1: 18000, lang: 'de' }
        ]
      } : null
    }),
  ...
  t.eq(ap.errors, [], 'Audio-Stufe (v7): keine Laufzeitfehler beim Laden');
  ...
  t.eq(ap.log, [], 'Web Speech wird NICHT bemüht, wenn eine Tonspur vorliegt (kein speechSynthesis.speak)');

  const audioEl = ap.win.__audioEl;
  t.ok(!!audioEl, 'HTML5-<audio>-Element wird erzeugt');
  t.ok(audioEl.getAttribute('src') === '/audio/articles/test-artikel.mp3', 'Korrekte MP3-Quelle gesetzt');
  ...
  t.ok(ap.doc.getElementById('ff-reader-status').textContent.length > 0,
    'Statusmeldung zur Wiedergabe wird gesetzt',
  ...
  t.ok(!!activeEl && (activeEl.closest('.ff-kurzantwort, .post-content, .ff-korrektur') !== null
    || activeEl.classList.contains('ff-reader-toolbar')),
    'Markierung sitzt auf einem echten Inhaltsblock',
    activeEl ? activeEl.tagName + '.' + activeEl.className : 'keine Markierung');
  t.ok(/noch ca\. \d+ Min\./.test(ap.doc.getElementById('ff-reader-remaining').textContent),
  ...
  // Abschnittssprung „weiter“ (Block 1 -> Block 2)
  const beforeJump = audioEl.currentTime;
  click(ap.win, ap.doc.getElementById('ff-listen-next'));
  await until(() => audioEl.currentTime > beforeJump + 1, 3000);
  ...
  /* Rückfall: ohne Tonspur nutzt dieselbe Seite die Web-Speech-Stufe –
     die Garantie-Stufe darf die Grundfunktion nie abschalten. */
  ...
  t.ok(noAudio.log.length > 0, 'Ohne Tonspur greift automatisch die Web-Speech-Stufe');
  t.ok(!noAudio.win.__audioEl, 'Ohne Tonspur wird kein <audio>-Element erzeugt');
}

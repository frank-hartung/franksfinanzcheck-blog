...
| 4 | **Befund korrigiert + Wache** | Gezählt wurde im Quell-Repo (`static/audio/` = 1 handgemachtes Paar). Die Tonspuren liegen auf `gh-pages/audio/articles/`: **18 von 31 Live-Artikeln haben eine Studio-Tonspur**, 13 nutzen die Gerätestimme. Neu: `scripts/audio_coverage_check.py` (Build-/Ref-modus, „unbekannt" ≠ „0 %"), Tagesschritt in `lesehilfen-gate.yml`, Selbsttest + `governance_contract.GUARDS` | Bei Bedarf einmalig Deploy mit `audio_backfill=true` auslösen (Runbook: `docs/AUDIO-BACKFILL-RUNBUCH.md` — Dauer, Queue-Falle, Kontrolle, Abbruch) (Input existiert in `deploy.yml`): mit Backfill entfällt die Stückgrenze – `audio_limit_new` wird ignoriert, der Generator läuft mit `--limit-new 0` – die 90-Minuten-Wanduhr ist die eigentliche Grenze; Fertiges bleibt im Cache, der Rest läuft im nächsten Lauf weiter |
